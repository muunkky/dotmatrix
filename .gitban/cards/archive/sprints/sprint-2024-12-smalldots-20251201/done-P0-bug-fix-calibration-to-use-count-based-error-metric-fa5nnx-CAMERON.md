...
- [x] Rewrite `calibrate_radius()` with count-based error metric
- [x] Implement binary search for min_radius (find tightest lower bound)
- [x] Implement binary search for max_radius (find tightest upper bound)
- [x] Update `CalibrationResult` to report count-based metrics
- [x] Update `format_calibration_output()` for new metrics
- [x] Update tests for new behavior
- [x] Verify on test_dotmatrix.png: should find bounds ~[116, 256] with error=0

## Success Criteria

- [x] Calibration achieves error=0 when all circles detected
- [x] Returns tightest valid bounds (not just "stabilized" bounds)
- [x] Binary search converges in O(log n) iterations per parameter
