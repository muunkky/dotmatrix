...
1. **Review Industry CLI Patterns**:
   - [x] Study Click best practices for option grouping
   - [x] Review similar tools (ImageMagick, ffmpeg, ruff) for UX patterns
   - [x] Document findings in ADR

2. **Design Decision: Default Behavior**:
   - [x] Decide: Should detection + JSON be default, or detection + extraction?
   - [x] User research: What do users expect from `dotmatrix -i image.png`?
   - [x] Document decision rationale

### Phase 2: Incremental Refactoring

3. **Extract _do_detect into smaller functions**:
   - [x] Create `_setup_detection()` for detection mode selection
   - [x] Create `_run_detection()` for actual detection
   - [x] Create `_handle_output()` for output/extraction logic
   - [x] Run tests after each extraction

4. **Add --mode preset option**:
   - [x] Implement `--mode standard|halftone|cmyk-sep` preset
   - [x] Map presets to existing flag combinations
   - [x] Add deprecation warning for replaced flags (optional)
   - [x] Run tests

5. **Restructure help output**:
   - [x] Group options using Click option groups
   - [x] Add examples section to help
   - [x] Add "For X use case..." guidance
   - [x] Run tests

6. **Review and adjust defaults**:
   - [x] Evaluate `--extract` default behavior
   - [x] Consider `--data-only` vs `--extract` semantics
...
7. **Final Validation**:
   - [x] All tests pass
   - [x] Manual testing of common workflows
   - [x] Help output review
   - [x] Backward compatibility verified

### Refactoring Techniques

- [x] **Extract Method**: _do_detect split into setup/run/output
- [x] **Introduce Parameter Object**: Detection config dataclass
- [x] **Replace Conditional with Polymorphism**: Detection mode strategies (optional)
...
- [x] **Capture Current Behavior**:
  - [x] All existing tests pass
  - [x] Document current default behavior
  - [x] Capture example outputs for comparison

### During Refactoring

- [x] **Test After Each Step**:
  - [x] Run full test suite after each commit
  - [x] No test should fail due to refactoring
  - [x] Add new tests for preset modes
...
- [x] **Comprehensive Verification**:
  ...
  - [x] Help output is readable and helpful
