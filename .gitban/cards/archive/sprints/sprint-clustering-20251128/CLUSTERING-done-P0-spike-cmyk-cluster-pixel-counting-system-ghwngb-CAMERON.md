...
- [x] Each cluster has exactly 1 black dot (via connected components)
- [x] Each cluster has 0-3 CMY midtone dots (when including RGB overlaps)
- [x] Pixel counts are accurate within 5% of manual measurement (verified via unit tests)
- [x] Handles edge cases (missing colors, partial overlaps)

## Implementation

Implemented in `src/dotmatrix/cluster_pixel_counter.py` with 19 passing tests.

CLI: `dotmatrix -i image.png --convex-edge --palette cmyk --cluster-count`

Commits:
- feat(cluster): add CMYK cluster pixel counting module
- feat(cli): add --cluster-count flag for pixel-level CMYK analysis
